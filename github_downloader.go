...
func InitGithubDownloader() {
	...
	IsDevInstall = os.Getenv("PLEXCORD_DEV_INSTALL") == "1"
	...
	PlexcordFile := PlexcordDirectory

	stat, err := os.Stat(PlexcordFile)
	if err != nil {
		return
	}
	...
	// dev
	if stat.IsDir() {
		PlexcordFile = path.Join(PlexcordFile, "main.js")
	}

	// Check hash of installed version if exists
	b, err := os.ReadFile(PlexcordFile)
	if err != nil {
		return
	}

	Log.Debug("Found existing Plexcord Install. Checking for hash...")
	re := regexp.MustCompile(`// Plexcord (\w+)`)
...
func installLatestBuilds() (retErr error) {
	...
	Log.Debug("Will save to:", PlexcordDirectory)
	...
	out, err := os.OpenFile(PlexcordDirectory, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
		Log.Error("Failed to create", PlexcordDirectory+":", err)
		retErr = err
		return
	}
	read, err := io.Copy(out, res.Body)
	if err != nil {
		Log.Error("Failed to download to", PlexcordDirectory+":", err)
		retErr = err
		return
	}
	...
	_ = FixOwnership(PlexcordDirectory)

	Log.Debug("Successfully downloaded desktop.asar to:", PlexcordDirectory)
	Log.Debug("Downloaded", read, "bytes")
