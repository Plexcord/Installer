...
if grep -q "CHROMEOS_RELEASE_NAME" /etc/lsb-release 2>/dev/null; then
	echo "ChromeOS is not supported! Use the chrome extension. https://chromewebstore.google.com/detail/plexcord-web/pdbajenofcbjlenlccdlfpdckjgooagi"
	exit 1
fi
...
curl -sS https://github.com/Plexcord/Installer/releases/latest/download/PlexcordInstallerCli-Linux \
  --output "$outfile" \
  --location \
  --fail
